...
export function ResetPasswordPage() {
  ...
  const [error, setError] = useState<string | null>(tokenError === 'INVALID_TOKEN' ? t('reset.invalid_token') : null);
          ...
          <div className="space-y-6">
            <div className="flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 dark:border-emerald-800 dark:bg-emerald-950">
              ...
              <div className="text-sm">
                <p className="font-medium text-emerald-800 dark:text-emerald-200">{t('reset.success_title')}</p>
                <p className="text-emerald-700 dark:text-emerald-300">{t('reset.success_desc')}</p>
              </div>
            </div>
            <Link to="/login" className="block text-center text-sm text-primary hover:underline">{t('reset.sign_in')}</Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <Label htmlFor="new-password">{t('reset.new_password')}</Label>
              <Input id="new-password" type="password" placeholder={t('sign_up.password_placeholder')} value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} />
              {password.length > 0 && (
                <ul className="space-y-1 pt-1">
                  <li className={`flex items-center gap-1.5 text-xs transition-colors ${password.length >= 8 ? 'text-emerald-600' : 'text-muted-foreground'}`}>
                    <Check size={12} className={password.length >= 8 ? 'opacity-100' : 'opacity-30'} />{t('reset.rule_length')}
                  </li>
                </ul>
              )}
            </div>
            <div className="space-y-2">
              ...
              <Input id="confirm-password" type="password" placeholder={t('reset.confirm_placeholder')} value={confirm} onChange={(e) => setConfirm(e.target.value)} required />
            </div>
            {error && <Alert variant="destructive">{error}</Alert>}
            <Button type="submit" disabled={loading} className="w-full h-11">{loading ? t('reset.resetting') : t('reset.reset')}</Button>
          </form>
        )}
      </div>
    </div>
