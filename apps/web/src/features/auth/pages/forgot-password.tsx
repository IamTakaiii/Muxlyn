      ...
      <div className="w-full max-w-[400px] space-y-8">
        ...
        {sent ? (
          <div className="space-y-6">
            <div className="flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 dark:border-emerald-800 dark:bg-emerald-950">
              ...
              <div className="text-sm">
                <p className="font-medium text-emerald-800 dark:text-emerald-200">{t('forgot.check_email')}</p>
                <p className="text-emerald-700 dark:text-emerald-300">If an account exists for <strong>{email}</strong>, a password reset link has been sent.</p>
              </div>
            </div>
            <Link to="/login" className="block text-center text-sm text-primary hover:underline">{t('forgot.back')}</Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Button type="submit" disabled={loading} className="w-full h-11">{loading ? t('forgot.sending') : t('forgot.send')}</Button>
            <Link to="/login" className="block text-center text-sm text-muted-foreground hover:text-primary transition-colors">{t('forgot.back')}</Link>
          </form>
        )}
      </div>
    </div>
