...
interface ToastItem { id: number; message: string; variant: 'success' | 'warning' | 'error'; }
...
export function ToastProvider() {
  ...
  const addToast = useCallback((message: string, variant: 'success' | 'warning' | 'error') => {
    ...
    const timer = setTimeout(() => { timersRef.current.delete(id); setToasts((prev) => prev.filter((t) => t.id !== id)); }, 5000);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2" aria-live="polite" aria-label="Notifications">
      {toasts.map((toast) => (
        <div key={toast.id} role="status" className={`flex items-center justify-between gap-3 rounded-md border px-4 py-3 shadow-lg ${VARIANT_STYLES[toast.variant] ?? VARIANT_STYLES.warning}`}>
          <p className="text-sm">{toast.message}</p>
          <button type="button" onClick={() => dismiss(toast.id)} className="text-current opacity-50 hover:opacity-100" aria-label="Close">&#x2715;</button>
        </div>
      ))}
    </div>
  );
